...
static int
shader_compiled_successfully(unsigned int shader, const char *filepath)
{
   int success;
...
static int
shader_linked_successfully(unsigned int program)
{
   int success;
...
static void
shader_bind_variable_names(unsigned int program)
{
   for (enum mesh_attribute i = MESH_ATTRIBUTE_POSITION;
        i < MESH_ATTRIBUTE_COUNT; ++i)
...
int
shader_load_from_file(struct shader *shader, const char *vpath,
                      const char *fpath)
{
...
void
shader_destroy(struct shader *shader)
{
