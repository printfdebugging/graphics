 ...
 */
void
mesh_load_vertices(struct mesh *mesh, void *data, unsigned int count,
                   unsigned int stride)
{
   glBindVertexArray(mesh->vao);
   glGenBuffers(1, &mesh->vbo_vertex);
   glBindBuffer(GL_ARRAY_BUFFER, mesh->vbo_vertex);
   glBufferData(GL_ARRAY_BUFFER, count * 3 * sizeof(float), data,
                GL_STATIC_DRAW);
   glVertexAttribPointer(MESH_ATTRIBUTE_POSITION, 3, GL_FLOAT, GL_FALSE, stride,
                         0);
...
void
mesh_load_indices(struct mesh *mesh, void *data, unsigned int count,
                  GLenum type, unsigned int stride)
{
   glBindVertexArray(mesh->vao);
   glGenBuffers(1, &mesh->ebo);
   glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, mesh->ebo);
   glBufferData(GL_ELEMENT_ARRAY_BUFFER, glTypeToSize(type) * count, data,
                GL_STATIC_DRAW);
...
void
mesh_load_colors(struct mesh *mesh, void *data, unsigned int count,
                 unsigned int stride)
{
   glBindVertexArray(mesh->vao);
   ...
   glBufferData(GL_ARRAY_BUFFER, count * 3 * sizeof(float), data,
                GL_STATIC_DRAW);
   glVertexAttribPointer(MESH_ATTRIBUTE_COLOR, 3, GL_FLOAT, GL_FALSE, stride,
                         0);
...
void
mesh_load_uv(struct mesh *mesh, void *data, unsigned int count,
             unsigned int stride)
{
   glBindVertexArray(mesh->vao);
   glGenBuffers(1, &mesh->vbo_uv);
   glBindBuffer(GL_ARRAY_BUFFER, mesh->vbo_uv);
   glBufferData(GL_ARRAY_BUFFER, count * 2 * sizeof(float), data,
                GL_STATIC_DRAW);
...
void
mesh_load_normals(struct mesh *mesh, void *data, unsigned int count,
                  unsigned int stride)
{
   glBindVertexArray(mesh->vao);
   ...
   glBufferData(GL_ARRAY_BUFFER, count * 3 * sizeof(float), data,
                GL_STATIC_DRAW);
   glVertexAttribPointer(MESH_ATTRIBUTE_NORMAL, 3, GL_FLOAT, GL_FALSE, stride,
                         0);
