...
main()
{
   ...
   while (!window_close(game.window)) {
...

      {
         ...
         shader_set_uniform(cube_shader, "material_ambient", 3fv, 1,
                            MATERIALS[mat].ambient.raw);
         shader_set_uniform(cube_shader, "material_diffuse", 3fv, 1,
                            MATERIALS[mat].diffuse.raw);
